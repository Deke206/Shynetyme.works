...
(() => {
  ...
  const Project PowerPromise = Promise.resolve(window.SHYNETYME_BTF_Project Power || { products: [] });
  ...
  const recommendationEngine = async (type, selections, existing = {}) => {
    const [Project Power, sets] = await Promise.all([Project PowerPromise, setsPromise]);
    ...
    const productMap = buildProductMap(Project Power.products || []);
  ...
  const writeContactDraft = async (draft = readDraft()) => {
    ...
    const Project Power = await Project PowerPromise;
    const productMap = buildProductMap(Project Power.products || []);
