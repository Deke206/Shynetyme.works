...
// BLE UUID and storage keys. UUIDs must remain aligned with the ESP32 firmware.
...
function normalizeDevice(d, i) {
  return {
    id: d?.id || uid("dev"),
    name: d?.name || `ESP32 ${i + 1}`,
    bluetoothId: d?.bluetoothId || null,
    bluetoothName: d?.bluetoothName || "",
    config: { ...clone(DEFAULT_CONFIG), ...(d?.config || {}) },
    lastFx: d?.lastFx || "RAINBOW",
...
let groups = (loadJSON(STW_GROUP_KEY, []) || []).map((g) => ({
  id: g.id || uid("grp"),
  name: g.name || "Group",
...
function targetMemberIds() {
  ...
  if (target.type === "group") {
    const g = groupById(target.id);
    return g ? g.members.filter((id) => deviceById(id)) : [];
  }
  return [];
}
...
function beginManualBluetooth() {
  ...
  for (const r of runtimes.values()) {
    if (r.auto && r.status === "connecting") {
      try {
        r.bt?.gatt?.disconnect();
      } catch (_) {}
      r.status = "assigned";
      r.server = r.cmd = r.st = null;
      r.auto = false;
    }
  }
...
async function writeRaw(id, text, fast = false) {
  ...
  const c = r.cmd,
    b = enc.encode(text);
  let op;
  const longPacket = b.length > 18;
  if (longPacket && c.properties?.write && c.writeValueWithResponse)
    op = c.writeValueWithResponse(b);
  else if (
    fast &&
    c.properties?.writeWithoutResponse &&
    c.writeValueWithoutResponse
  )
    op = c.writeValueWithoutResponse(b);
  else if (c.properties?.write && c.writeValueWithResponse)
    op = c.writeValueWithResponse(b);
  else if (c.writeValue) op = c.writeValue(b);
...
function queueWrite(id, text, fast = false) {
  const r = runtime(id);
  const run = async () => {
    for (const part of chunks(text)) await writeRaw(id, part, fast);
    return true;
  };
  r.queue = r.queue.then(run, run).catch((e) => {
    r.error = e.message;
    emit("tx-error", { deviceId: id, message: e.message });
    return false;
  });
  return r.queue;
}
...
function parseStatus(text) {
  ...
  for (const p of String(text || "").split(";")) {
    const n = p.indexOf("=");
    if (n > 0) o[p.slice(0, n)] = p.slice(n + 1);
  }
  return o;
}
...
async function readStatus(id) {
  ...
  try {
    ...
    await sleep(100);
    const v = await waitFor(r.st.readValue(), 1800, "Status read timed out"),
      o = parseStatus(dec.decode(v));
    r.lastStatus = { ...r.lastStatus, ...o };
    const d = deviceById(id);
    if (d) {
      if (o.CFGLEDS || o.LEDS)
        d.config.leds = +(o.CFGLEDS || o.LEDS) || d.config.leds;
      if (o.CFGPIN || o.PIN) d.config.gpio = +(o.CFGPIN || o.PIN);
      if (o.ORDER) d.config.order = o.ORDER;
      if (o.FX) d.lastFx = o.FX;
      saveDevices();
    }
    emit("status", { deviceId: id, status: o });
    return o;
  } catch (e) {
    r.error = e.message;
    emit("status-error", { deviceId: id, message: e.message });
    return null;
  }
}
...
async function refreshGranted() {
  ...
  try {
    ...
    for (const d of list) granted.set(d.id, d);
    emit("granted");
    return list;
  } catch (e) {
    emit("granted-error", { message: e.message });
    return [];
  }
}
...
function attachDisconnect(id, bt) {
  ...
  if (r.disconnectHandler)
    try {
      bt.removeEventListener("gattserverdisconnected", r.disconnectHandler);
    } catch (_) {}
  r.disconnectHandler = () => {
    r.status = "assigned";
    r.server = r.cmd = r.st = null;
    r.queue = Promise.resolve();
    r.auto = false;
...
async function connectAssigned(
  ...
  if (!bt) {
    ...
    bt = granted.get(d.bluetoothId);
  }
  if (!bt) {
    r.status = "assigned";
    r.error = "Bluetooth permission is not available for this saved device";
  ...
  try {
    await withConnectLock(async () => {
      ...
      r.cmd = await waitFor(
        svc.getCharacteristic(STW_COMMAND_UUID),
        auto ? 1800 : 2400,
        "Control characteristic not found",
      );
      r.st = await waitFor(
        svc.getCharacteristic(STW_STATUS_UUID),
        auto ? 1800 : 2400,
    ...
    r.status = "connected";
    r.auto = false;
    d.bluetoothName = bt.name || d.bluetoothName;
    saveDevices();
    ...
    r.status = "assigned";
    r.auto = false;
    r.error = e.message === "Auto reconnect cancelled" ? "" : e.message;
    try {
      r.server?.disconnect();
    } catch (_) {}
    r.server = r.cmd = r.st = null;
    if (e.message !== "Auto reconnect cancelled")
      emit("connect-error", { deviceId: id, message: e.message });
    return false;
  }
}
...
async function autoConnect() {
  ...
  if (!ids.length) {
    const first = devices.find(
      (d) => d.bluetoothId && granted.has(d.bluetoothId),
    );
    ids = first ? [first.id] : [];
  }
  for (const id of ids) {
...
function disconnectDevice(id) {
  ...
  r.status = deviceById(id)?.bluetoothId ? "assigned" : "unassigned";
  r.auto = false;
  r.server = r.cmd = r.st = null;
  r.queue = Promise.resolve();
  emit("disconnected", { deviceId: id });
...
function removeLogicalDevice(id) {
  ...
  groups = groups
    .map((g) => ({ ...g, members: g.members.filter((x) => x !== id) }))
...
function createGroup(name) {
  ...
  const g = { id: uid("grp"), name: clean, members: [] };
  groups.push(g);
  saveGroups();
  emit("groups");
  return g.id;
...
function validateConfig(c) {
  ...
  if (!Number.isFinite(c.gpio) || c.gpio < 0 || c.gpio > 39)
    return "GPIO must be 0–39";
  if (!["RGB", "GRB", "BRG", "GBR", "RBG", "BGR"].includes(c.order))
    return "Invalid pixel order";
  if (c.segFrom < 0 || c.segTo < c.segFrom || c.segTo >= c.leds)
...
async function saveDeviceConfig(id, cfg, { reboot = false } = {}) {
  ...
  await sendToDevice(id, `LEDS=${c.leds};PIN=${c.gpio};ORDER=${c.order};SAVE`);
...
async function togglePower(id) {
  ...
  if (d.powered) {
    ...
    await sendToDevice(
      id,
      `FX=${d.lastFx && d.lastFx !== "OFF" ? d.lastFx : "RAINBOW"}`,
    );
    d.powered = true;
  }
  saveDevices();
