...
function runtime(id){if(!runtimes.has(id))runtimes.set(id,{bt:null,server:null,cmd:null,st:null,status:'idle',queue:Promise.resolve(),lastStatus:{},disconnectHandler:null,error:''});return runtimes.get(id)}
...
function attachDisconnect(id,bt){const r=runtime(id);if(r.disconnectHandler)try{bt.removeEventListener('gattserverdisconnected',r.disconnectHandler)}catch(_){}r.disconnectHandler=()=>{r.status='assigned';r.server=r.cmd=r.st=null;r.queue=Promise.resolve();emit('disconnected',{deviceId:id})};bt.addEventListener('gattserverdisconnected',r.disconnectHandler)}
async function openGatt(bt){if(bt.gatt?.connected)return bt.gatt;let last;for(let n=0;n<2;n++){try{await sleep(n?420:160);return await waitFor(bt.gatt.connect(),5500,'Bluetooth connection timed out')}catch(e){last=e;try{bt.gatt?.disconnect()}catch(_){}}}throw last||new Error('Bluetooth connection failed')}
async function connectAssigned(id,{auto=false}={}){const d=deviceById(id),r=runtime(id);if(!d?.bluetoothId)throw new Error('No Bluetooth device assigned');if(connected(id))return true;let bt=granted.get(d.bluetoothId);if(!bt){await refreshGranted();bt=granted.get(d.bluetoothId)}if(!bt){r.status='assigned';r.error='Bluetooth permission is not available for this saved device';emit('connect-missing',{deviceId:id});return false}r.bt=bt;r.status='connecting';r.error='';emit('connecting',{deviceId:id,auto});try{await withConnectLock(async()=>{attachDisconnect(id,bt);r.server=await openGatt(bt);const svc=await waitFor(r.server.getPrimaryService(STW_SERVICE_UUID),3500,'ShyneTyme BLE service not found');r.cmd=await waitFor(svc.getCharacteristic(STW_COMMAND_UUID),2600,'Control characteristic not found');r.st=await waitFor(svc.getCharacteristic(STW_STATUS_UUID),2600,'Status characteristic not found')});r.status='connected';d.bluetoothName=bt.name||d.bluetoothName;saveDevices();emit('connected',{deviceId:id,auto});setTimeout(()=>readStatus(id),220);return true}catch(e){r.status='assigned';r.error=e.message;try{r.server?.disconnect()}catch(_){}r.server=r.cmd=r.st=null;emit('connect-error',{deviceId:id,message:e.message});return false}}
async function assignGranted(id,btId){const d=deviceById(id);if(!d)throw new Error('Unknown device');await refreshGranted();const bt=granted.get(btId);if(!bt)throw new Error('That Bluetooth device is not granted to this page');const owner=devices.find(x=>x.id!==id&&x.bluetoothId===bt.id);if(owner)throw new Error(`That Bluetooth device is already assigned to ${owner.name}`);d.bluetoothId=bt.id;d.bluetoothName=bt.name||'Bluetooth device';saveDevices();emit('assigned',{deviceId:id});return connectAssigned(id)}
async function assignNew(id){if(!navigator.bluetooth)throw new Error('Web Bluetooth is not available');if(!window.isSecureContext)throw new Error('HTTPS is required for Bluetooth');const bt=await navigator.bluetooth.requestDevice({acceptAllDevices:true,optionalServices:[STW_SERVICE_UUID]});granted.set(bt.id,bt);const d=deviceById(id);if(!d)throw new Error('Unknown device');const owner=devices.find(x=>x.id!==id&&x.bluetoothId===bt.id);if(owner)throw new Error(`That Bluetooth device is already assigned to ${owner.name}`);d.bluetoothId=bt.id;d.bluetoothName=bt.name||'Bluetooth device';saveDevices();emit('assigned',{deviceId:id});return connectAssigned(id)}
async function autoConnect(){await refreshGranted();for(const d of devices){if(d.bluetoothId&&granted.has(d.bluetoothId))await connectAssigned(d.id,{auto:true})}if(!target){const first=devices.find(d=>connected(d.id));if(first)target={type:'device',id:first.id}}saveTarget();emit('autoconnect-complete')}
function disconnectDevice(id){const r=runtime(id);try{r.server?.disconnect()}catch(_){}r.status=deviceById(id)?.bluetoothId?'assigned':'unassigned';r.server=r.cmd=r.st=null;r.queue=Promise.resolve();emit('disconnected',{deviceId:id})}
...
function unassignBluetooth(id){const d=deviceById(id);if(!d)return false;disconnectDevice(id);d.bluetoothId=null;d.bluetoothName='';runtime(id).status='idle';saveDevices();if(target?.type==='device'&&target.id===id)clearTarget();emit('unassigned',{deviceId:id});return true}
...
queueMicrotask(()=>{emit('ready');setTimeout(autoConnect,250)});
