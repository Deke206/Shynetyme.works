...
let ai=0,micOn=false,ctx,an,stream,raf,lastTx=0,lastBeat=0,sl=0,sb=0,smid=0,sh=0;
...
function syncColorUI(cmd){for(const part of cmd.split(';')){const n=part.indexOf('=');if(n<1)continue;const k=part.slice(0,n),v=part.slice(n+1);if(k==='FG'&&$('fg'))$('fg').value='#'+v;if(k==='BG'&&$('bg'))$('bg').value='#'+v;if(k==='MAIN'&&$('main'))$('main').value='#'+v;if(k==='BRI'&&$('bri')){$('bri').value=v;if($('briV'))$('briV').textContent=v}if(k==='SPD'&&$('spd')){$('spd').value=v;if($('spdV'))$('spdV').textContent=v}if(k==='INT'&&$('int')){$('int').value=v;if($('intV'))$('intV').textContent=v}}}
...
if($('sync'))$('sync').onchange=()=>{if($('target'))$('target').textContent=$('sync').checked?'TARGET BOTH':'TARGET #'+(ai+1)};
document.querySelectorAll('[data-fx]').forEach(b=>b.onclick=()=>{stopPlaylist();document.querySelectorAll('[data-fx]').forEach(x=>x.classList.remove('on'));b.classList.add('on');send('FX='+b.dataset.fx)});
if($('fg'))$('fg').onchange=e=>{stopPlaylist();send('FG='+e.target.value.slice(1).toUpperCase())};
